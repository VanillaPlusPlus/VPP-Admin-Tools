...
class ChatBasedAdminTool extends PluginBase
{
	...
	{
		Print("ChatBasedAdminTool:: OnInit():: Loading ChatBasedAdminTool Mods...");
	...
	void RequestHandler( Param request_info )
	{
		...
		if (chat_params)
		{
			...
			for (int i = 0; i < players.Count(); ++i)
			{
				if (players.Get(i).GetIdentity().GetName() == chat_params.param2 && PermitManager.Cast(GetPluginManager().GetPluginByType(PermitManager)).IsSuperAdmin(players.Get(i).GetIdentity().GetPlainId()))
				{
					Admin 		  = players.Get(i);
						...
						case "/strip":
							...
							case "/tpto":
								...
								if (m_TPLocations.Contains(cData))
								{
									...
									ofixPlayerPos = g_Game.SnapToGround( ofixPlayerPos );
							...
							case "/tpc":
								...
								fixPlayerPos = g_Game.SnapToGround( fixPlayerPos );
