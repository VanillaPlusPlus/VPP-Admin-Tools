...
class BansManager extends ConfigurablePlugin
{
	...
	override void Load()
	{
		...
		m_BanList.Insert(new BannedPlayer("user","00000000000000000","123456789",new BanDuration(0,0,0,0,0,true),"Example ban","Example reason"));
		Save();
	}
	...
	void UpdateBanDuration(CallType type, ParamsReadContext ctx, PlayerIdentity sender, Object target)
	{
		...
		if(type == CallType.Server)
		{
			...
			{
				BannedPlayer entry = GetBannedPlayer(id);
				if (entry != null)
				{
					//Update Ban Duration
	...
	*/
	void RemoveFromBanList(string id)
	{
		...
		BannedPlayer player = GetBannedPlayer(id);
		if (player != null)
		{
	...
	{
		BannedPlayer bannedPlayer = GetBannedPlayer(id);
	...
	*/
	...
	{
		BanDuration currentTimeStamp = GetCurrentTimeStamp();
	...
	*/
	...
	//Works with steam64,GUID & name
	BannedPlayer GetBannedPlayer(string id)
	{
