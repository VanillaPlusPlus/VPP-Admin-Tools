class XMLEditor extends PluginBase
{
	...
	void GetDetails(CallType type, ParamsReadContext ctx, PlayerIdentity sender, Object target)
	{
	 	...
	 	if ( type == CallType.Server )
		{
			...
			{
				autoptr ref array<ref Param3<string,string,int>> result = new array<ref Param3<string,string,int>>;
	...
	{
		ref Param2<string, ref array<ref Param3<string,string,int>>> data;
		if (!ctx.Read(data)) return;
	 	...
	 	if ( type == CallType.Server )
		{
			...
			if (parser.Load() && parser.VerifyXml())
			{
				foreach(ref Param3<string,string,int>> params: paramsData)
				{
