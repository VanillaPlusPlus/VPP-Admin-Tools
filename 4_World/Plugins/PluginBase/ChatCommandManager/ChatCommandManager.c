			...
			if(command.GetMinArgumentCount() == 0 && args.Count() == 0)
			{
				switch(command.GetCommand())
				{
					...
					case "/refuel": 
						...
						Car toBeFilled;
						EntityAI carEntity;
						GetGame().GetObjectsAtPosition3D( self.GetPosition(), 10, nearest_objects, null );
						...
						for (i = 0; i < nearest_objects.Count(); i++) 
						{
							if (nearest_objects.Get(i).IsKindOf("CarScript")) 
							{
								...
								carEntity  = EntityAI.Cast(nearest_objects[i]);
								carEntity.SetHealth("GlobalHealth", "Health",carEntity.GetMaxHealth());
								...
								if (vehParts != null){
									foreach(EntityAI att : vehParts){
										if (att != null)
											att.SetHealth("GlobalHealth", "Health",att.GetMaxHealth());
									}
								}
								...
								GetPermissionManager().NotifyPlayer(id,nearest_objects.Get(i).GetType() + ": reparied & refueled [" +fuelReq+ "L] added, all fluids maxed",NotifyTypes.NOTIFY);
							}
						}
					break;
				}
