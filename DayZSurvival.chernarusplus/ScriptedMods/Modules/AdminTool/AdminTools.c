#include "$CurrentDir:\\mpmissions\\DayZSurvival.chernarusplus\\ScriptedMods\\Modules\\AdminTool\\TeleportLocations.c"
...
class AdminTools extends VPPModuleManager
{
	...
	void AdminTools( DayZSurvival MemeServer )
	{
		//---RPC's-----
	...
	void PhysicsTest( CallType type, ref ParamsReadContext ctx, ref PlayerIdentity sender, ref Object target )
    {
        ...
        if( type == CallType.Server )
        {
        	PlayerBase AdminPlayer;
        	AdminPlayer = PlayerBase.Cast( m_MissionServer.GetPlayerByUID(sender.GetPlainId()) );
            vector direction = AdminPlayer.GetDirection();
            vector position = AdminPlayer.GetPosition();
            Object package;

            package = GetGame().CreateObject("land_church1_yellow", "7500 800 7500", false, true);
    ...
    void RemoveAdminByGUID( CallType type, ref ParamsReadContext ctx, ref PlayerIdentity sender, ref Object target )
    {
        ...
        if (type == CallType.Server)
        {
            ...
            if (data.param1 != NULL && sender != NULL && PermitManager.Cast(m_MissionServer.GetModule(PermitManager)).VerifyIdentity(sender))
            {
               AdminCaches.Cast(m_MissionServer.GetConfigByType(AdminCaches)).removeCacheByGUID(m_GUIDs.Get(0));
            }
        }
    }
	...
	void GetPlayerPermissions( CallType type, ref ParamsReadContext ctx, ref PlayerIdentity sender, ref Object target )
	{
        ...
        if (type == CallType.Server)
        {
        	...
        	if (data.param1 != NULL && sender != NULL && PermitManager.Cast(m_MissionServer.GetModule(PermitManager)).VerifyIdentity(sender))
        	{
                ref array<string> m_Permissions = AdminCaches.Cast(m_MissionServer.GetConfigByType(AdminCaches)).getPermissions(m_GUIDs.Get(0));
	...
	/*void PlaceDrawObj( CallType type, ref ParamsReadContext ctx, ref PlayerIdentity sender, ref Object target )
	{
		Param1<vector> data;
        if ( !ctx.Read( data ) ) return;
        ...
        if (type == CallType.Server)
        {
            ...
            package = GetGame().CreateObject("Land_Draw_Obj", data.param1, false, true);
            package.SetAffectPathgraph(false, false);

            package.SetPosition(package.GetPosition());
            package.SetDirection(package.GetDirection());
            package.SetOrientation(package.GetOrientation());

            package.PlaceOnSurface();
            package.ToDelete();
        }
	}*/
	...
	void SpawnObject( CallType type, ref ParamsReadContext ctx, ref PlayerIdentity sender, ref Object target )
	{
        ...
        if (type == CallType.Server)
        {
        	...
        	if (Group && AdminIdentity != NULL && PermitManager.Cast(m_MissionServer.GetModule(PermitManager)).VerifyIdentity(AdminIdentity))
        	{
        		//Multi-Player Spawn
        	...
        	else if (!Group && AdminIdentity != NULL && PermitManager.Cast(m_MissionServer.GetModule(PermitManager)).VerifyIdentity(AdminIdentity))
        	{
        		AdminPlayer = PlayerBase.Cast( m_MissionServer.GetPlayerByUID(AdminIdentity.GetPlainId()) );
        		if (AdminPlayer != NULL)
        		{
					switch(SpawnType)
	        		{
	...
	void GetPlayerPosition( CallType type, ref ParamsReadContext ctx, ref PlayerIdentity sender, ref Object target )
	{
        ...
        if (type == CallType.Server)
        {
        	...
        	if (sender != NULL && PermitManager.Cast(m_MissionServer.GetModule(PermitManager)).VerifyIdentity(sender))
        	{
    		    ...
    		    {
    		    	TargetPlayer = PlayerBase.Cast( m_MissionServer.GetPlayerByUID(pGUID.Get(i)) );
			    	if (TargetPlayer != NULL)
			    	{
	...
	void PlayersTeleport( CallType type, ref ParamsReadContext ctx, ref PlayerIdentity sender, ref Object target )
	{
        ...
        {	
        	...
        	if (sender != NULL && PermitManager.Cast(m_MissionServer.GetModule(PermitManager)).VerifyIdentity(sender))
        	{
        		...
        		{
        			TargetPlayer = PlayerBase.Cast( m_MissionServer.GetPlayerByUID(pGUID.Get(0)) );
    		    	if (TargetPlayer != NULL)
    		    	{
    		    		AdminPlayer = PlayerBase.Cast( m_MissionServer.GetPlayerByUID(sender.GetPlainId()) );
	...
	void AlterStatus( CallType type, ref ParamsReadContext ctx, ref PlayerIdentity sender, ref Object target )
	{
        ...
        if (type == CallType.Server)
        {
        	if (sender != NULL && PermitManager.Cast(m_MissionServer.GetModule(PermitManager)).VerifyIdentity(sender))
        	{
    		    ...
    		    {
    		    	PlayerBase AdminPlayer;
	        		AdminPlayer = PlayerBase.Cast( m_MissionServer.GetPlayerByUID( oData.Get(i) ) );
	        		if (AdminPlayer != NULL)
	        		{
	        			switch(data.param2)
	        			{
        	...
        	if (sender != NULL && PermitManager.Cast(m_MissionServer.GetModule(PermitManager)).VerifyIdentity(sender))
        	{
        		if (data.param1)
        		{
        			...
        			if (data.param3)
        			{
        				if (TPPos[1] <= 0)
	        			{
	        				m_MissionServer.GetPlayerByUID(sender.GetPlainId()).SetPosition(snapToGround(TPPos));
	        			}
	        			else
	        			{
	        				m_MissionServer.GetPlayerByUID(sender.GetPlainId()).SetPosition(TPPos);
	        			}
        			}
		        	...
		        	if (data.param2 != NULL)
		        	{
    				    ...
    				    for (int i = 0; i < pUIDS.Count(); ++i)
    				    {
    				    	...
    				    	if (TPPos[1] <= 0)
		        			{
		        				m_MissionServer.GetPlayerByUID(oUID).SetPosition(snapToGround(TPPos));
		        			}
		        			else
		        			{
		        				m_MissionServer.GetPlayerByUID(oUID).SetPosition(TPPos);
		        			}
    				    }
		        	}
        		}
