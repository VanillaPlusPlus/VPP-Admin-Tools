...
{
    private ref map<string, ref TextWidget> m_Stats;
    ...
    {
        m_Stats = new map<string, ref TextWidget>;
	...
	string GetStatVlaue(string name)
	{
		foreach(string stat, string statValue : m_stats)
        {
			if (stat == name)
			{
				return statValue;
