class MenuBansManager extends AdminHudSubMenu
{
	...
	override bool OnClick(Widget w, int x, int y, int button)
	{
		...
		{
			case m_SelectAll:
			foreach(ref BannedPlayerEntry entry : m_Entries)
			{
	...
	void EditBanReason(int result, string input)
	{
		if (result == DIAGRESULT.OK && input != "")
		{
			...
			autoptr array<string> IDs = new array<string>();
	...
	*/
	...
	void HandleData(CallType type, ParamsReadContext ctx, PlayerIdentity sender, Object target)
	{
		if(type == CallType.Client)
		{
			...
			autoptr array<ref BannedPlayer> bannedPlayers = data.param1;
	...
	void AddEntry(BannedPlayer player)
	{
		...
		if(m_LastGrid.GetContentCount() < 100)
		{
			ref BannedPlayerEntry entry = new BannedPlayerEntry(m_LastGrid.GetGrid(), player);
	...
	void RemoveSelectedBans(int result)
	{
		if (result == DIAGRESULT.YES)
		{
			...
			if(selected.Count() > 1)
			{
				...
				foreach(ref BannedPlayerEntry entry : selected)
				{
					if (entry != null)
					{
						ids.Insert(entry.GetPlayer().Steam64Id);
	...
	{
		autoptr array<ref BannedPlayerEntry> selected = new array<ref BannedPlayerEntry>;
		foreach(ref BannedPlayerEntry entry : m_Entries)
		{
	...
	void UpdateFilter()
	{
        ...
        {
            autoptr BannedPlayerEntry entry = m_Entries.Get(x);            
