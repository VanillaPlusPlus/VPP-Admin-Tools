class PresetItemEntry: VPPPlayerTemplate
{
	...
	void PresetItemEntry(GridSpacerWidget grid, ref PresetItemData data, string typeName, bool isParent)
	{
		m_typeName    = typeName;
		m_IsParent    = isParent;
	...
	override bool OnClick(Widget w, int x, int y, int button)
	{
		if (w == m_RemoveItem && button == MouseState.LEFT)
		{
			...
			SetEdited(true);
			autoptr MenuItemManager iManager = MenuItemManager.Cast(VPPAdminHud.Cast(GetVPPUIManager().GetMenuByType(VPPAdminHud)).GetSubMenuByType(MenuItemManager));
			if (iManager != null)
